    ...
    <el-card>
      ...
      <el-form ref="requestParams"
        ...
        <el-row :gutter="22"
          ...
          <el-col :span="8"
                  v-for="(item,index) in processFormItemData.requestInfo"
                  :key="index">
            <el-form-item :label="item.title"
                          label-width="120px"
                          :prop="'requestInfo['+item.field+']'"
                          :rules="item.required ? [{required: true, message: '不能为空', trigger: 'change'}]:[{required: false}]">
              <!-- 普通输入框 -->
              <div v-if="item.type === 'input'"
                   style="display: flex;">
                <el-input v-model="processParamsData.requestInfo[item.field]"
              ...
              <treeselect :options="item.data"
                          v-model="processParamsData.requestInfo[item.field]"
                          clearable
              ...
              <!-- 下拉框 -->
              <el-select v-model="processParamsData.requestInfo[item.field]"
              ...
              <!-- 时间 -->
              <el-date-picker v-model="processParamsData.requestInfo[item.field]"
                              v-if="item.type === 'date'"
                              :disabled="item.disabled"
                              type="date"
                              placeholder="选择日期"
